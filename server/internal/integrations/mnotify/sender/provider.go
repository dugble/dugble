package sender

import (
	"context"
	"fmt"

	"github.com/dugble/dugble/server/internal/integrations/mnotify"
	platformsenderid "github.com/dugble/dugble/server/internal/messaging/senderids/provider"
)

const (
	ProviderID = platformsenderid.ProviderMNotify
	createPath = "/api/senderid/register"
	statusPath = "/api/senderid/status"
)

type Provider struct{ client *mnotify.Client }

var _ platformsenderid.Provider = (*Provider)(nil)

func NewProvider(client *mnotify.Client) *Provider { return &Provider{client: client} }
func (provider *Provider) ID() string              { return ProviderID }

func (provider *Provider) Create(ctx context.Context, request CreateRequest) (*CreateResponse, error) {
	if provider == nil || provider.client == nil {
		return nil, fmt.Errorf("mNotify Sender ID creation failed: %w", mnotify.ErrClientUnavailable)
	}
	request = request.Normalize()
	if err := validateCreateRequest(request); err != nil {
		return nil, fmt.Errorf("mNotify Sender ID creation failed: %w", err)
	}

	var response createResponse
	if err := provider.client.Post(ctx, createPath, newCreateRequest(request), &response); err != nil {
		return nil, fmt.Errorf("mNotify Sender ID creation failed: %w", err)
	}
	mapped, err := mapCreateResponse(request.SenderID, &response)
	if err != nil {
		return nil, fmt.Errorf("mNotify Sender ID creation failed: %w", err)
	}
	return mapped, nil
}

func (provider *Provider) CheckStatus(ctx context.Context, senderID string) (*StatusResponse, error) {
	if provider == nil || provider.client == nil {
		return nil, fmt.Errorf("mNotify Sender ID status check failed: %w", mnotify.ErrClientUnavailable)
	}
	if err := platformsenderid.ValidateName(senderID); err != nil {
		return nil, fmt.Errorf("mNotify Sender ID status check failed: %w", err)
	}

	var response statusResponse
	if err := provider.client.Post(ctx, statusPath, newStatusRequest(senderID), &response); err != nil {
		return nil, fmt.Errorf("mNotify Sender ID status check failed: %w", err)
	}
	mapped, err := mapStatusResponse(senderID, &response)
	if err != nil {
		return nil, fmt.Errorf("mNotify Sender ID status check failed: %w", err)
	}
	return mapped, nil
}
