...
func (handler *Handler) Get(c *echo.Context) error {
	item, err := handler.service.Get(c.Request().Context(), c.Param("allowance_id"))
	if err != nil {
		return httputil.Error(c, err)
	}
...
func (handler *Handler) Close(c *echo.Context) error {
	...
	item, err := handler.service.Close(c.Request().Context(), c.Param("allowance_id"), input)
	if err != nil {
		return httputil.Error(c, err)
	}
	return httputil.OK(c, item)
...
func (handler *Handler) Replace(c *echo.Context) error {
	...
	item, err := handler.service.Replace(c.Request().Context(), c.Param("allowance_id"), input)
	if err != nil {
		return httputil.Error(c, err)
	}
	return httputil.Created(c, item)
