...
func TestSendPreservesAlphanumericSender(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["from"] != "Acme" { t.Fatalf("from = %q", body["from"]) }
		w.WriteHeader(http.StatusAccepted)
	...
	if err != nil || result.State != sms.SubmissionAccepted { t.Fatalf("result=%+v err=%v", result, err) }
}

func TestSendTreatsAcceptedWithMalformedBodyAsAccepted(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { w.WriteHeader(http.StatusAccepted); _, _ = w.Write([]byte(`{not-json`)) }))
	defer server.Close()
	provider, err := vonage.New(vonage.Config{APIKey: "api-key", APISecret: "secret", BaseURL: server.URL})
	if err != nil { t.Fatal(err) }
	result, err := provider.Send(context.Background(), sms.Message{To: "+233200000000", From: "Acme", Text: "hello"})
...
func TestSendClassifiesClientErrorsAsRejected(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { w.WriteHeader(http.StatusUnprocessableEntity); _, _ = w.Write([]byte(`{"type":"https://developer.vonage.com/api-errors#invalid-request","title":"Invalid Request","detail":"invalid to"}`)) }))
	defer server.Close()
	provider, err := vonage.New(vonage.Config{APIKey: "api-key", APISecret: "secret", BaseURL: server.URL})
	if err != nil { t.Fatal(err) }
	result, err := provider.Send(context.Background(), sms.Message{To: "+233200000000", From: "Acme", Text: "hello"})
	if err == nil || result.State != sms.SubmissionRejected { t.Fatalf("result=%+v err=%v", result, err) }
	var apiErr *vonage.APIError
	if !errors.As(err, &apiErr) || apiErr.Detail != "invalid to" { t.Fatalf("error = %#v", err) }
}

func TestSendClassifiesRateLimitAsRejected(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { w.WriteHeader(http.StatusTooManyRequests); _, _ = w.Write([]byte(`{"type":"https://developer.vonage.com/api-errors#throttled","title":"Too Many Requests","detail":"rate limited"}`)) }))
	defer server.Close()
	provider, err := vonage.New(vonage.Config{APIKey: "api-key", APISecret: "secret", BaseURL: server.URL})
	if err != nil { t.Fatal(err) }
	result, err := provider.Send(context.Background(), sms.Message{To: "+233200000000", From: "Acme", Text: "hello"})
...
func TestSendTreatsRequestTimeoutAsUnknown(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { w.WriteHeader(http.StatusRequestTimeout); _, _ = w.Write([]byte(`{"title":"Request Timeout","detail":"request timed out"}`)) }))
	defer server.Close()
	provider, err := vonage.New(vonage.Config{APIKey: "api-key", APISecret: "secret", BaseURL: server.URL})
	if err != nil { t.Fatal(err) }
	result, err := provider.Send(context.Background(), sms.Message{To: "+233200000000", From: "Acme", Text: "hello"})
	if err == nil || result.State != sms.SubmissionUnknown { t.Fatalf("result=%+v err=%v", result, err) }
}

func TestSendClassifiesServerErrorsAsUnknown(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { w.WriteHeader(http.StatusInternalServerError); _, _ = w.Write([]byte(`{"title":"Internal Error","detail":"try later"}`)) }))
	defer server.Close()
	provider, err := vonage.New(vonage.Config{APIKey: "api-key", APISecret: "secret", BaseURL: server.URL})
	if err != nil { t.Fatal(err) }
	result, err := provider.Send(context.Background(), sms.Message{To: "+233200000000", From: "Acme", Text: "hello"})
...
func TestCapabilitiesRequireE164Recipient(t *testing.T) {
	...
	if !capabilities.AlphanumericSenderID || !capabilities.RequiresE164Recipient { t.Fatalf("unexpected capabilities: %+v", capabilities) }
}

func TestNewRequiresCredentials(t *testing.T) {
	_, err := vonage.New(vonage.Config{})
	if !errors.Is(err, vonage.ErrInvalidConfig) { t.Fatalf("New() error = %v, want ErrInvalidConfig", err) }
}
