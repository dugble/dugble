...
type deliveryRepository interface {
	...
	MarkSubmitted(context.Context, uuid.UUID, uuid.UUID, uuid.UUID, awsses.Result) error
...
func NewProcessor(repository deliveryRepository, sender awsses.Sender) *Processor {
	return &Processor{
...
func NewHandler(repository deliveryRepository, sender awsses.Sender) *Processor {
...
func (processor *Processor) Handle(ctx context.Context, command DeliverCommand) error {
	...
	route, applicationHeaders := awsses.ExtractDeliveryRoute(message.Headers)
	result, err := processor.sender.Send(ctx, awsses.Message{
		...
		From:             awsses.Address{Email: message.FromEmail, Name: message.FromName},
