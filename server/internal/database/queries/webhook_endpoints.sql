...
SELECT team.id, sqlc.arg(url), sqlc.arg(signing_secret), true, sqlc.arg(subscribed_events)
FROM teams AS team
WHERE team.id = sqlc.arg(team_id) AND team.status = 'active'
RETURNING *;

-- name: ListWebhookEndpoints :many
SELECT endpoint.* FROM webhook_endpoints AS endpoint
JOIN teams AS team ON team.id = endpoint.team_id
WHERE endpoint.team_id = sqlc.arg(team_id) AND team.status = 'active'
ORDER BY endpoint.created_at DESC LIMIT sqlc.arg(limit_count) OFFSET sqlc.arg(offset_count);

-- name: GetWebhookEndpoint :one
SELECT endpoint.* FROM webhook_endpoints AS endpoint
JOIN teams AS team ON team.id = endpoint.team_id
WHERE endpoint.id = sqlc.arg(id) AND endpoint.team_id = sqlc.arg(team_id) AND team.status = 'active';
...
SET url = sqlc.arg(url),
    ...
    disabled_at = CASE WHEN sqlc.arg(enabled)::boolean THEN NULL ELSE COALESCE(endpoint.disabled_at, now()) END,
    ...
    updated_at = now()
FROM teams AS team
WHERE endpoint.id = sqlc.arg(id) AND endpoint.team_id = sqlc.arg(team_id)
  AND team.id = endpoint.team_id AND team.status = 'active'
RETURNING endpoint.*;
...
SELECT endpoint.* FROM webhook_endpoints AS endpoint
JOIN teams AS team ON team.id = endpoint.team_id
WHERE endpoint.team_id = sqlc.arg(team_id) AND endpoint.enabled = true
  AND endpoint.disabled_at IS NULL AND sqlc.arg(event_type)::text = ANY(endpoint.subscribed_events)
