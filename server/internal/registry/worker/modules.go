...
func (registry *Registry) newModules(startupCtx context.Context) (modules, error) {
	...
	broadcastExecutionRepository := broadcastmodule.NewRepositoryWithEventEmitter(db, platformevent.NewEmitter(platformwebhook.NewEventSink(webhookEmitter)))
	broadcastExecutionJob := broadcastmodule.NewJob(broadcastexecution.NewProcessor(broadcastExecutionRepository), broadcastmodule.JobConfig{PollInterval: time.Second, BatchSize: 100})

	emailSender, err := awsses.NewSESSender(startupCtx, cfg.AWS.Region, cfg.AWS.FromEmail, cfg.AWS.AccessKey, cfg.AWS.SecretKey, platformemail.TransactionalConfigurationSet)
	if err != nil { return modules{}, fmt.Errorf("initialize SES email sender: %w", err) }
	emailDeliveryProcessor := emaildelivery.NewProcessor(emaildelivery.NewRepository(db), emailSender)
	transactionalEmailConsumer := emaildelivery.NewConsumer(messagingClient, processedEvents, emailDeliveryProcessor, emaildelivery.ConsumerConfig{Stream: "transactional", Concurrency: 5, AckWait: 2*time.Minute, HandlerTimeout: 45*time.Second, MaxDeliver: 6, RetryPolicy: emaildelivery.DefaultRetryPolicy()})
	marketingEmailConsumer := emaildelivery.NewConsumer(messagingClient, processedEvents, emailDeliveryProcessor, emaildelivery.ConsumerConfig{Stream: "marketing", Concurrency: 2, AckWait: 2*time.Minute, HandlerTimeout: 45*time.Second, MaxDeliver: 6, RetryPolicy: emaildelivery.DefaultRetryPolicy()})
	systemEmailConsumer := systememail.NewConsumer(messagingClient, processedEvents, emailSender, systememail.ConsumerConfig{Concurrency: 3, AckWait: time.Minute, HandlerTimeout: 30*time.Second, MaxDeliver: 6})
	systemEmailRenderer, err := systemmail.NewRenderer()
	if err != nil { return modules{}, fmt.Errorf("initialize system email renderer: %w", err) }
	...
	emailTenantConsumer := emailtenant.NewProvisioningConsumer(messagingClient, processedEvents, emailtenant.NewProvisioningProcessor(emailTenantRepository, emailSender), emailtenant.ProvisioningConsumerConfig{Concurrency: 3, AckWait: 2*time.Minute, HandlerTimeout: 60*time.Second, RetryBackOff: emailtenant.DefaultProvisioningRetryBackOff()})
	...
	emailFeedbackConsumer := emailfeedback.NewConsumer(messagingClient, processedEvents, emailfeedback.NewHandlerWithMetrics(emailFeedbackRepository, feedbackMetrics), emailfeedback.ConsumerConfig{Concurrency: 5, AckWait: time.Minute, HandlerTimeout: 30*time.Second, MaxDeliver: 6, RetryPolicy: emailfeedback.DefaultRetryPolicy()})
	emailFeedbackReconciler := emailfeedback.NewObservedReconciler(emailFeedbackRepository, emailfeedback.ReconcilerConfig{PollInterval: 5*time.Second, BatchSize: 25, Concurrency: 5, LeaseDuration: 2*time.Minute, HandleTimeout: 30*time.Second}, feedbackMetrics)
	...
	domainJob, err := domainmodule.NewJob(db, domainRepository, domainService, domainmodule.DefaultJobConfig(), "sender-domain-reconciliation-"+uuid.NewString())
	if err != nil { return modules{}, fmt.Errorf("initialize sender domain reconciliation: %w", err) }
	...
	domainClaimJob, err := domainclaimmodule.NewJob(domainClaimRepository, domainClaimService, domainclaimmodule.DefaultJobConfig(), "sender-domain-claim-reconciliation-"+uuid.NewString())
	if err != nil { return modules{}, fmt.Errorf("initialize domain claim reconciliation: %w", err) }
	...
	if err != nil { return modules{}, fmt.Errorf("initialize communication routes: %w", err) }
	smsSender, err := platformsms.NewRelayService(routes, smsProviders...)
	if err != nil { return modules{}, fmt.Errorf("initialize SMS sender: %w", err) }

	senderIDRun := disabledSenderIDReconciliation
	if len(senderIDProviders) != 0 {
		senderIDJob, jobErr := senderidmodule.NewJob(senderidmodule.NewRepository(db), senderidmodule.DefaultJobConfig(), "sender-id-reconciliation-"+uuid.NewString(), senderIDProviders...)
		if jobErr != nil { return modules{}, fmt.Errorf("initialize Sender ID reconciliation: %w", jobErr) }
	...
	smsCampaignSMSService := smsmodule.NewService(smsRepository, nil, smsdelivery.NewQueue(outboxRepository), platformbilling.NewService(platformbilling.NewRepository(db)))
	smsCampaignJob := smscampaignmodule.NewJob(smscampaignexecution.NewProcessor(smscampaignmodule.NewRepository(db), smsCampaignSMSService), smscampaignmodule.JobConfig{PollInterval: time.Second, BatchSize: 100})
	smsConsumer := smsdelivery.NewConsumer(messagingClient, processedEvents, smsdelivery.NewProcessor(smsdelivery.NewRepository(db, smsRepository), smsSender), smsdelivery.ConsumerConfig{Concurrency: 10, AckWait: 2*time.Minute, HandlerTimeout: 45*time.Second, MaxDeliver: 6})
	...
	smsFeedbackReconciler := smsfeedback.NewReconciler(smsFeedbackRepository, smsSender, smsFeedbackProcessor, smsfeedback.ReconcilerConfig{BatchSize: 100, Concurrency: 10, ProviderTimeout: 15*time.Second})
	...
	webhookProcessor := webhookdelivery.NewProcessor(webhookRepository, webhookdelivery.NewClient(10*time.Second), webhookdelivery.DefaultRetryPolicy(), webhookWorkerID).WithNotifier(notificationEmailService)
	webhookConsumer := webhookdelivery.NewConsumer(webhookRepository, webhookProcessor, webhookdelivery.ConsumerConfig{PollInterval: 500*time.Millisecond, BatchSize: 50, Concurrency: 10, LockTimeout: 30*time.Second, HandleTimeout: 15*time.Second}, webhookWorkerID)

	renewalService := subscriptionRenewal.NewService(subscriptionRenewal.NewRepository(), chargeSubscription.NewService(chargeSubscription.NewRepository()), subscriptionLifecycle.NewService()).WithEventPublisher(subscriptionRenewal.NewEventPublisher(outboxRepository))
	...
	renewalConfig.OnFailure = func(ctx context.Context, failure subscriptionRenewal.Failure) { sentrymonitoring.ErrorContext(ctx, "subscription renewal failed", "team_id", failure.TeamID, "error", failure.Err) }
	renewalWorker, err := subscriptionRenewal.NewWorker(db, renewalService, renewalConfig)
	if err != nil { return modules{}, fmt.Errorf("initialize subscription renewal worker: %w", err) }
...
func newCommunicationRoutes(providers []relaysms.Provider) (*relaycore.RouteTable, error) {
	...
	for _, provider := range providers { routes = append(routes, relaycore.Route{Provider: provider.Name(), CountryCode: "GH", Priority: len(routes)+1, Enabled: true}) }
