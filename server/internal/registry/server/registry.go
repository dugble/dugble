...
func New(ctx context.Context) (*Registry, error) {
	if ctx == nil { return nil, errors.New("server registry context is required") }
	registry := &Registry{}
	fail := func(err error) (*Registry, error) { registry.Close(); return nil, err }
	cfg, err := config.Load()
	if err != nil { return fail(fmt.Errorf("load configuration: %w", err)) }
	registry.config = cfg
	if err := sentrymonitoring.Init(cfg.Sentry, cfg.AppEnv); err != nil { return fail(fmt.Errorf("initialize Sentry: %w", err)) }
	...
	if err != nil { return fail(fmt.Errorf("initialize New Relic client: %w", err)) }
	...
	registry.cleanups.Add(func() { if closeErr := registry.redis.Close(); closeErr != nil { sentrymonitoring.Warn("close Redis client", "error", closeErr) } })
	registry.arcjet, err = arcjetadapter.New(cfg.ArcjetKey)
	if err != nil { return fail(fmt.Errorf("initialize Arcjet client: %w", err)) }
	registry.emailClient, err = newEmailClient(cfg)
	if err != nil { return fail(fmt.Errorf("initialize email client: %w", err)) }
...
func (registry *Registry) Run(ctx context.Context) error {
	if registry == nil || registry.server == nil { return errors.New("server registry is not configured") }
	return registry.server.Run(ctx)
}
func (registry *Registry) Close() { if registry != nil { registry.cleanups.Run() } }

func (registry *Registry) routerConfig() httptransport.RouterConfig {
	return httptransport.RouterConfig{Development: registry.config.IsDevelopment(), CORSOrigins: registry.config.CORSOrigins, Arcjet: registry.arcjet, BodyLimit: platformemail.MaxHTTPRequestBytes, Idempotency: httpmiddleware.IdempotencyConfig{Repository: idempotency.NewRepository(registry.postgres)}, Middleware: defaultHTTPMiddleware()}
}

func newEmailClient(cfg *config.Config) (*awsses.Client, error) { return awsses.NewClient(cfg.AWS.Region, cfg.AWS.FromEmail, cfg.AWS.AccessKey, cfg.AWS.SecretKey, platformemail.TransactionalConfigurationSet) }
func newHubtelServices(cfg *config.Config, db *pgxpool.Pool) (*hubtel.Provider, *paymentmodule.Service) {
	if !cfg.Hubtel.Enabled { return nil, nil }
...
func newSystemEmailQueue(cfg *config.Config, repository *outbox.Repository) *systememail.Queue { return systememail.NewQueue(repository, platformemail.Message{Provider: awsses.ProviderSES, Region: cfg.AWS.Region, Stream: "transactional", ConfigurationSet: platformemail.TransactionalConfigurationSet, SESTenantName: platformemail.SystemSESTenantName}) }
func newProviderSNSHandler(cfg *config.Config, db *pgxpool.Pool, repository *outbox.Repository) *providersns.Handler {
	if len(cfg.AWS.SNSTopicARNs) == 0 { return nil }
...
func newSMSProviders(cfg *config.Config) ([]relaysms.Provider, error) {
	...
	if cfg.Moolre.VASKey != "" {
		...
		if err != nil { return nil, fmt.Errorf("initialize Moolre provider: %w", err) }
		providers = append(providers, provider)
	}
	if cfg.Sendexa.Token != "" {
		provider, err := sendexaprovider.New(sendexaprovider.Config{Token: cfg.Sendexa.Token})
		if err != nil { return nil, fmt.Errorf("initialize Sendexa provider: %w", err) }
		providers = append(providers, provider)
	}
	if len(providers) == 0 { return nil, fmt.Errorf("initialize SMS providers: configure MOOLRE_VAS_KEY or SENDEXA_TOKEN") }
...
func newSMSRouteTable(providers []relaysms.Provider) (*relaycore.RouteTable, error) {
	...
	for _, provider := range providers {
		...
		routes = append(routes, relaycore.Route{Provider: provider.Name(), CountryCode: "GH", Priority: priority, Enabled: true})
	}
...
func newSMSSender(cfg *config.Config) (*platformsms.RelayService, error) {
	...
	if err != nil { return nil, fmt.Errorf("initialize SMS routes: %w", err) }
	sender, err := platformsms.NewRelayService(routes, providers...)
	if err != nil { return nil, fmt.Errorf("initialize SMS sender: %w", err) }
	return sender, nil
}

type cleanupStack struct{ functions []func() }
func (stack *cleanupStack) Add(cleanup func()) { if stack != nil && cleanup != nil { stack.functions = append(stack.functions, cleanup) } }
