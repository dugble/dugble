...
func TestNextVerificationDelayKeepsPendingChecksFast(t *testing.T) {
	...
	config := JobConfig{PendingCheckInterval: 30 * time.Second, HealthCheckInterval: 24 * time.Hour}

	want := jitter(config.PendingCheckInterval, id)
	for _, attempt := range []int32{1, 2, 5, 20} {
		if got := nextVerificationDelay(StatusPending, nil, attempt, id, config); got != want {
...
func TestNextVerificationDelayBacksOffOnlyOnErrors(t *testing.T) {
	...
	for _, attempt := range []int32{1, 2, 3, 6} {
		...
		got := nextVerificationDelay(StatusPending, errors.New("provider unavailable"), attempt, id, config)
		if got != want {
...
func TestNextVerificationDelayMovesVerifiedDomainsToHealthCadence(t *testing.T) {
	...
	if got := nextVerificationDelay(StatusVerified, nil, 12, id, config); got != want {
