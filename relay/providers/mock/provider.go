...
func New(name string, send SendFunc) *Provider {
...
func (p *Provider) Send(ctx context.Context, message sms.Message) (sms.SendResult, error) {
	...
	p.mu.Unlock()
	if p.send == nil {
		return sms.SendResult{State: sms.SubmissionUnknown}, nil
	}
...
func (p *Provider) Messages() []sms.Message {
	p.mu.Lock()
	defer p.mu.Unlock()
	messages := make([]sms.Message, len(p.messages))
